...
static void ADC_Callback(ADC_Handle_T *pHander, ADC_Event_T event, unsigned long param)
{
    ...
    {
        adc_status_idle = 1;
        PR_DEBUG("\nAsync ADC Sequence Convert Success\n");
        for (i = 0; i < SEQUENCE_LENGTH; i++)
        {
            PR_DEBUG("IndexChannle:%d Vaule:%d\n", s_sequence[i], s_convertValue[i]);
        }
    }
...
int main(void)
{
    ...
    /*commonConfig.trigerSource = PES_EVENT_CHANNEL0;*/
    commonConfig.trigerMode = ADC_SW_TRIGER;
#ifdef TEST_ADC_PES_CASE30
