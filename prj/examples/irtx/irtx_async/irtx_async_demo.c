...
******************************************************************************/
int main(void);
static void IRTX_Callback(IRTX_Handle_T *hIRTX, IRTX_Event_T event, unsigned long param);
...
static void IRTX_Callback(IRTX_Handle_T *hIRTX, IRTX_Event_T event, unsigned long param)
{
    if (event == IRTX_EVEN_SEND_SUCCESS)
    {
        PR_DEBUG("Async Send Success\n");
    }
...
int main(void)
{
    ...
    /* send key every 2 s*/
    ret = IRTX_SendKey(&s_Handle, &keyDesc, TIMEOUT_NO_BLOCKING);
    if (ret != RET_ONGOING)
    {
