...
RET_CODE OSC_SetConfig(OSC_Config_T *pConfig)
{
    ...
    /* Configure internal low oscillator */
...
RET_CODE CLOCK_SetConfig(CLOCK_Config_T *pConfig)
{
    ...
    if (pConfig->clkMask & CLOCK_MASK_SYSCLK)
    {
        ...
        IO_BITMASK_SET(FLASH->CTRL, 3 << 16, 2 << 16);
        /* Configure CPU clock prescaler */
        IO_BITMASK_SET(SYSDEV->CLK0, 0xFF0, (pConfig->sysClkPrescaler << 4));
...
RET_CODE CLOCK_GetConfig(CLOCK_Config_T *pConfig)
{
    ...
    /*Get CPU clock source*/
    pConfig->sysClkSource    = (CLOCK_SysClockSource_T)(SYSDEV->SCS >> 16 & 0x07);
...
RET_CODE PeriphClk_SetConfig(PeriphClk_Config_T *pConfig)
{
    ...
    if ((pConfig->periphClkMask & CLOCK_PERIPH_RTC2_MASK) == CLOCK_PERIPH_RTC2_MASK)
    {
        ...
        if (pConfig->rtc2Clock == RTC2_CLOCK_IRCLS)
        {
        ...
        else if (pConfig->rtc2Clock == RTC2_CLOCK_OSCLS)
        {
        ...
        else if (pConfig->rtc2Clock == RTC2_CLOCK_PLLSRC || pConfig->rtc2Clock == RTC2_CLOCK_PLL6X)
        {
            /*Check whether PLL is powered down*/
            if (SYSDEV->PLL & SYS_PLL_PDB)
            {
    ...
    {
        if (pConfig->rtc1Clock == RTC1_CLOCK_IRCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LIRC32K);
            while (!Clock_IsNowStable(IRC1K_SOURCE_STABLE_MASK))
            {
        ...
        else if (pConfig->rtc1Clock == RTC1_CLOCK_OSCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LOSC32K);
            while (!Clock_IsNowStable(OSC32K_SOURCE_STABLE_MASK))
            {
        ...
        else if (pConfig->rtc1Clock == RTC1_CLOCK_PLLSRC || pConfig->rtc1Clock == RTC1_CLOCK_PLL6X)
        {
            /*Check whether PLL is powered down*/
            if (SYSDEV->PLL & SYS_PLL_PDB)
            {
    ...
    {
        if (pConfig->rtc0Clock == RTC0_CLOCK_IRCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LIRC32K);
            while (!Clock_IsNowStable(IRC1K_SOURCE_STABLE_MASK))
            {
        ...
        else if (pConfig->rtc0Clock == RTC0_CLOCK_OSCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LOSC32K);
            while (!Clock_IsNowStable(OSC32K_SOURCE_STABLE_MASK))
            {
        ...
        else if (pConfig->rtc0Clock == RTC0_CLOCK_PLLSRC || pConfig->rtc0Clock == RTC0_CLOCK_PLL6X)
        {
            /*Check whether PLL is powered down*/
            if (SYSDEV->PLL & SYS_PLL_PDB)
            {
    ...
    if ((pConfig->periphClkMask & CLOCK_PERIPH_RTC2_MASK) == CLOCK_PERIPH_RTC2_MASK)
    {
        if (pConfig->rtc2Clock == RTC2_CLOCK_OSCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LOSC32K);
            while (!Clock_IsNowStable(LOSC32K_SOURCE_STABLE_MASK))
            {
    ...
    if ((pConfig->periphClkMask & CLOCK_PERIPH_RTC1_MASK) == CLOCK_PERIPH_RTC1_MASK)
    {
        if (pConfig->rtc1Clock == RTC1_CLOCK_OSCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LOSC32K);
            while (!Clock_IsNowStable(LOSC32K_SOURCE_STABLE_MASK))
            {
    ...
    if ((pConfig->periphClkMask & CLOCK_PERIPH_RTC0_MASK) == CLOCK_PERIPH_RTC0_MASK)
    {
        if (pConfig->rtc0Clock == RTC0_CLOCK_OSCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LOSC32K);
            while (!Clock_IsNowStable(LOSC32K_SOURCE_STABLE_MASK))
            {
    ...
    {
        if (pConfig->calendarClock == RTC0_CLOCK_OSCLS)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LOSC32K);
            while (!Clock_IsNowStable(LOSC32K_SOURCE_STABLE_MASK))
            {
        ...
        else if (pConfig->calendarClock == CALENDAR_CLOCK_LIRC)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_LIRC32K);
            while (!Clock_IsNowStable(LIRC32K_SOURCE_STABLE_MASK))
            {
        ...
        else if (pConfig->calendarClock == CALENDAR_CLOCK_HIRC)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_HIRC8M);
            while (!Clock_IsNowStable(HIRC8M_SOURCE_STABLE_MASK))
            {
                __asm("nop");
            }
        }
        else if (pConfig->calendarClock == CALENDAR_CLOCK_HOSC)
        {
            SysClock_Enable(SYSCLOCK_SOURCE_HOSC8M);
            while (!Clock_IsNowStable(HOSC8M_SOURCE_STABLE_MASK))
            {
                __asm("nop");
            }
...
#define HOSC_STABLE_MASK            (1<<26)
        ...
        if (pConfig->rtc0Clock == RTC0_CLOCK_LOSC)
        {
            LOSC_OSCPIL_SEL_H;
            ...
            while (!(OSC_STABLE_REG & (LOSC_STABLE_MASK)));
        }
        else if (pConfig->rtc0Clock == RTC0_CLOCK_HOSC)
        {
    ...
    if ((pConfig->periphClkMask & CLOCK_PERIPH_LVD_MASK) == CLOCK_PERIPH_LVD_MASK)
    {
        /* Configure clock for lvd */
        CLOCK_PERIPH_LVD_CONFIG(pConfig->lvdClock);
    }

    if ((pConfig->periphClkMask & CLOCK_PERIPH_ACMP_MASK) == CLOCK_PERIPH_ACMP_MASK)
    {
        /* Configure clock for acmp */
        CLOCK_PERIPH_ACMP_CONFIG(pConfig->acmpClock);
    }

    if ((pConfig->periphClkMask & CLOCK_PERIPH_LCD_MASK) == CLOCK_PERIPH_LCD_MASK)
    {
        /* Configure clock for lcd */
        CLOCK_PERIPH_LCD_CP_CONFIG(pConfig->lcd_config_t.cpClock);
        CLOCK_PERIPH_LCD_CONFIG(pConfig->lcd_config_t.lcdClock);
    }
...
RET_CODE PeriphClk_GetConfig(PeriphClk_Config_T *pConfig)
{
...
#endif
    /* Get clock for lcd */
    pConfig->lcdClock = ((SYSDEV->CLK2) >> CLOCK_SEL2_LCD_POS) & 0x03;
#endif
    ...
    pConfig->adc_config_t.adcSrcClock   = ((SYSDEV->CLK2) >> CLOCK_SEL2_ADC_SRC_POS) & 0x01;
    ...
    pConfig->irtx_config_t.irtxDivider = ((SYSDEV->CLK2) >> CLOCK_SEL2_IRTX_DIV_POS) & 0x3F;
    /* Get clock for i2c1 */
    pConfig->i2c1Clock = ((SYSDEV->CLK1) >> CLOCK_SEL1_I2C1_POS) & 0x07;
    /* Get clock for i2c0 */
    pConfig->i2c0Clock = ((SYSDEV->CLK1) >> CLOCK_SEL1_I2C0_POS) & 0x07;
