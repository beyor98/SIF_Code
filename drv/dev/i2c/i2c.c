...
#if defined(CHIP_GM6601) || defined(CHIP_GM6611) || defined(CHIP_GM6611B) || defined(CHIP_GM6601B) || defined(CHIP_GM6621) || defined(CHIP_F6601C)
 ...
 * @brief Configure DMA for I2C peripheral
 *
 * @param pI2C A pointer to I2C handler.
 * @para  dmaWrite Indicate a DMA write/read.
 * @para  i2cMode I2C working mode, eihter master or slave mode.
 * @para  pDMAHandle A pointer to DMA operation handler.
 */
static RET_CODE I2C_DMAConfig(I2C_Device_T *pI2C, uint8_t dmaWrite, uint8_t i2cMode, DMA_Handle_T *hDMA)
{
    ...
    pDMAConfig->callback = DMA_CommonCallback;

    if (dmaWrite)
    {
        pDMAConfig->requestType = ((pI2C == I2C0) ? DMA_REQUEST_I2C0_TX :
                                   ((pI2C == I2C1) ? DMA_REQUEST_I2C1_TX :
                                    (DMA_REQUEST_INVALID)));
    ...
    {
        pDMAConfig->requestType = ((pI2C == I2C0) ? DMA_REQUEST_I2C0_RX :
                                   ((pI2C == I2C1) ? DMA_REQUEST_I2C1_RX :
                                    (DMA_REQUEST_INVALID)));
...
static RET_CODE I2C_InitHandle(I2C_Device_T *pI2C, I2C_Config_T *pConfig, I2C_Handle_T *hI2C)
{
    ...
    /* set tx fifo thrd to 2*/
    I2C_SET_FIFOTHRD_TX(hI2C->pDevice, 0x2);
    hI2C->txThreshld   = I2C_GET_FIFOTHRD_TX(hI2C->pDevice);
    hI2C->txFifoEmptyDepth = MAX_TX_FIFO_DEPTH - hI2C->txThreshld;
...
static RET_CODE I2C_ConfigBaudRate(I2C_Handle_T *hI2C, I2C_Config_T *pConfig)
{
    ...
    if (pConfig->workMode == I2C_MASTER_MODE)
    {
        ...
        {
            clk_div = ((IO_BITS_GET(*(volatile uint32_t *)0x40000044, (0x7 << 18))) >> 18) & 0x07;
        }
        if (hI2C->pDevice == I2C1)
        {
            clk_div = ((IO_BITS_GET(*(volatile uint32_t *)0x40000044, (0x7 << 21))) >> 21) & 0x07;
        }
        if (hI2C->pDevice == I2C2)
        {
            clk_div = ((IO_BITS_GET(*(volatile uint32_t *)0x40000044, (0x7 << 24))) >> 24) & 0x07;
        }
        clkFrq = SYS_CLOCK / (clk_div + 1);
#endif
    ...
    hI2C->pDevice->SH = 0x80070005;
    hI2C->pDevice->SH = 0x00070005;
 ...
 * @param write Indicate I2C read or write.
 */
static void I2C_DataCmdRegManipulate_Data_Cmd_Null(I2C_Handle_T *hI2C, I2C_DataCmdType_T dataCmdType, uint8_t *value, uint8_t size, I2C_RdWrType_T write)
{
    uint8_t i = 0;
    ASSERT(hI2C != NULL);
    for (i = 0; i < size; i++)
    {
        if (write == I2C_DATACMD_WRITE)
        {
            if (dataCmdType == I2C_DATA_CMD_NULL)
            {
                *((volatile uint16_t *)(&(hI2C->pDevice->DT))) = *(value + i);
            }
        }
        else
        {
...
#else//CHIP_GM6621
    /* Enable DMA correpoding bitfields of I2C peripheral */
    if (!(s_dmaHandle.pDevice->DMA_Chn[s_dmaHandle.hsChannelIdx].HSCFG & DMA_HSCFG_DH))
    {
        I2C_DMACTRL_TXEN_ENABLE(hI2C->pDevice);
    }
    if (!(s_dmaHandle.pDevice->DMA_Chn[s_dmaHandle.hsChannelIdx].HSCFG & DMA_HSCFG_SH))
    {
        I2C_DMACTRL_RXEN_ENABLE(hI2C->pDevice);
    }
#endif
...
RET_CODE I2C_Init(I2C_Device_T *pI2C, I2C_Config_T *pConfig, I2C_Handle_T *hI2C)
{
    ...
    {
        /*reset i2c*/
        read_data = READ_REG(*(volatile uint32_t *) 0x40000020);
        read_data &= ~(1 << 0);
        WRITE_REG(*(volatile uint32_t *) 0x40000020, read_data);
            ...
            if (intStatus & I2C_IS_TFAE)
            {
                ...
                if (hI2C->enableDMA != TRUE)
                {
                    ...
                    else
                    {
                        ...
                        if ((hI2C->txRemained - 1) >= hI2C->txFifoEmptyDepth)
                        {
                            // send hI2C->txFifoEmptyDepth data to fifo
                            size = hI2C->txFifoEmptyDepth;
                            I2C_DataCmdRegManipulate_Data_Cmd_Null(hI2C, I2C_DATA_CMD_NULL, hI2C->pTxBuffer, hI2C->txFifoEmptyDepth, I2C_DATACMD_WRITE);
                            hI2C->pTxBuffer = hI2C->pTxBuffer + hI2C->txFifoEmptyDepth;
                        }
                        else
                        {
                            // send (hI2C->txRemained -1) - hI2C->txFifoEmptyDepth data to fifo
                            size = hI2C->txRemained - 1;
                            I2C_DataCmdRegManipulate_Data_Cmd_Null(hI2C, I2C_DATA_CMD_NULL, hI2C->pTxBuffer, size, I2C_DATACMD_WRITE);
                            hI2C->pTxBuffer = hI2C->pTxBuffer + size;
                        }
                        hI2C->txRemained = hI2C->txRemained - size;
                    }
