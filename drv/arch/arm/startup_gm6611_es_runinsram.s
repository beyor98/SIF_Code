...
;BOOTSEL1_B0_1_B1_0  EQU     1 ; boot from main flash
;BOOTSEL1_B0_1_B1_1  EQU     1 ; boot from main flash
;BOOTSEL1_B0_0_B1_1  EQU     1 ; boot from info-flash
;BOOTSEL1_B0_0_B1_0  EQU     1 ; boot from sram
;BOOTSEL0_BP_1_B1_0  EQU     1 ; boot from sram
;BOOTSEL0_BP_1_B1_1  EQU     1 ; boot from info-flash
;BOOTSEL0_BP_0_B1_1  EQU     1 ; boot from main flash
;BOOTSEL0_BP_0_B1_0  EQU     1 ; boot from main flash
...
__Vectors_Size 	EQU     __Vectors_End - __Vectors   
                ...
                DCB     0xFF,       0xFF,       0xFF,       0xFF     ; Low -> High   0x400
                DCB     0xFF,       0xFF,       0xFF,       0xFF     ; Low -> High   0x404
                DCB     0xFF,       0xFF,       0xFF,       0xFF     ; Low -> High   0x408
                ; === 0x400[2:0]--> Boot1, Boot0, Boot_Selection 
                ;boot from main flash
                IF      :DEF:BOOTSEL1_B0_1_B1_0
				DCB     0xFB,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C
                ;boot from main flash
                ELIF      :DEF:BOOTSEL1_B0_1_B1_1
				DCB     0xFF,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C
                ;boot from info-flash    
                ELIF      :DEF:BOOTSEL1_B0_0_B1_1
				DCB     0xFD,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C
                ;boot from sram
                ELIF      :DEF:BOOTSEL1_B0_0_B1_0
				DCB     0xF9,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C
                ;boot from sram, BP: an external PIN, can be 0 or 1
                ELIF      :DEF:BOOTSEL0_BP_1_B1_0  ;does not care about B0
				DCB     0xF8,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C , does not care about boot0, 0xFA or F8
                ;boot from info-flash
                ELIF      :DEF:BOOTSEL0_BP_1_B1_1
				DCB     0xFC,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C , does not care about boot0, 0xFE or 0xFC
                ;boot from main flash
                ELIF      :DEF:BOOTSEL0_BP_0_B1_1
				DCB     0xFC,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C , does not care about boot0, 0xFE or 0xFC
                ;boot from main flash    
                ELIF      :DEF:BOOTSEL0_BP_0_B1_0
				DCB     0xF8,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C , does not care about boot0, 0xFA or 0xF8
                ;default value
                ELSE
                DCB     0xFF,       0xFF,       0xFF,       0xFF     ; Low -> High   0x40C
				ENDIF
                
                DCD     0xFFFFFFFF      ;0x410
                DCD     0xFFFFFFFF      ;0x414
                DCD     0xFFFFFFFF      ;0x418
                DCD     0xFFFFFFFF      ;0x41C
                ENDIF
...
Reset_Handler   PROC
                ...
                ;Remapped to 32K to 0x00000000~ 0x00000000 + 32K
                ;MOVS  R2, #0x1
				;MOVS  R0, R2, LSL#30 
                ;ADDS  R0, #0x14
                ;LDR   R2, [R0, #0]								
...
;label1
                ...
                ;LDR   R0, =0x4000014C
                ;MOVS  R1, #0x00
                ;STRB  R1, [R0, #0x1] ;configure PinMUX for GPIO function
                ;LDR   R0, =0x40013010
                ;MOVS  R1, #0xFF
                ;STRB  R1, [R0, #0x0] ;enable GPIO output
                ;LDR   R0, =0x40013004
                ;MOVS  R1, #0x30                
                ;STRB  R1, [R0, #0x0] ;output high                
