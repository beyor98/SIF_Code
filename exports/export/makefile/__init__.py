...
class Makefile(Exporter):
    ...
    def generate(self):
        ...
        for templatefile in ['makefile/%s_%s.tmpl' % (self.TEMPLATE, self.target.lower())] + ['makefile/%s_%s.tmpl' % (self.TEMPLATE, label.lower()) 
            for label in self.toolchain.target.extra_labels] + ['makefile/%s.tmpl' % self.TEMPLATE]:
            try:
