...
from exports.config import Config
from abc import ABCMeta, abstractmethod
from distutils.spawn import find_executable

from multiprocessing import Pool, cpu_count
from exports.utils import run_cmd, mkdir, rel_path, ToolException, NotSupportedException, split_path, compile_worker
from exports.settings import MBED_ORG_USER
import exports.hooks as hooks
...
class mbedToolchain:
    ...
    CORTEX_SYMBOLS = {
        ...
        "Cortex-A9" : ["__CORTEX_A9", "ARM_MATH_CA9", "__FPU_PRESENT", "__EVAL"],
    }
    ...
    def get_symbols(self, for_asm=False):
        if for_asm:
            ...
            if self.cxx_symbols is None:
                ...
                # self.cxx_symbols.extend(['CHIP_GM6611', 'SYS_CLOCK=48000000', 'DEBUG_PRINTF', '__REDLIB__', '__USE_CMSIS'])
                self.cxx_symbols.extend(['CHIP_GM6611', '__REDLIB__', '__USE_CMSIS'])
                if MBED_ORG_USER:
    ...
    def _add_dir(self, path, resources, base_path, exclude_paths=None):
        ...
        for root, dirs, files in walk(path, followlinks=True):
            ...
            for d in copy(dirs):
                ...
                #terrylee
                elif (d == 'prj' or d == 'bsp' or d == 'doc' or d == 'exports' or d == 'ucosii' or d == 'tool' or d == "Driver"):
                   dirs.remove(d)

                #elif (root == '..\\rabbit' and (d == 'bsp' or d == 'doc' or d == 'exports' or d == 'ucosii' or d == 'tool')):
                #   dirs.remove(d)
                #elif (root == '..\\rabbit\\prj' and (d == 'Eval' or d == 'testbench')):
                #    dirs.remove(d)
                #elif (root == '..\\rabbit\\drv' and d == 'cmsis'):  # terrylee add this line
                #    dirs.remove(d)
                #elif (root == '..\\rabbit\\prj\\examples' and d != 'uart'):
                #    dirs.remove(d)
                elif exclude_paths:
            ...
            root = root.rstrip("/")  # '..\\rabbit\\drv\\cmsis\\Driver\\Include'
    ...
    def _add_file(self, file_path, resources, base_path, exclude_paths=None):
        ...
        terry_basename = basename(file_path)

        if   ext == '.s':
            resources.s_sources.append(file_path)

        elif (terry_basename == 'irtx.c' or terry_basename == 'kbi.c' or terry_basename == 'lptim.c' or terry_basename == 'pes.c'): #terrylee add this
            pass;
        ...
        elif terry_basename == '.hgignore':
            resources.repo_files.append(file_path)

        elif terry_basename == '.gitignore':
            resources.repo_files.append(file_path)
...
from exports.settings import ARM_PATH
from exports.settings import GCC_ARM_PATH
from exports.settings import IAR_PATH
...
from exports.toolchains.arm import ARM_STD, ARM_MICRO
from exports.toolchains.gcc import GCC_ARM
from exports.toolchains.iar import IAR
